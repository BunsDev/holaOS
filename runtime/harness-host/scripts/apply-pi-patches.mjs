...
patchFile("node_modules/@mariozechner/pi-coding-agent/dist/core/compaction/compaction.js", [
  {
    verify: "const SUMMARIZATION_TOOL_RESULT_CHAR_BUDGET = 2000;",
    pattern:
...
const SUMMARIZATION_TOOL_RESULT_CHAR_BUDGET = 2000;
const SUMMARIZATION_ESTIMATED_BYTES_PER_TOKEN = 2;
const COMPACTION_IMAGE_PLACEHOLDER = "[image omitted during compaction]";
...
function serializeTextBlocks(blocks, maxCharsPerBlock) {
    ...
    const placeholderPrefix = content ? "\\n\\n" : "";
    const placeholder = \`\${placeholderPrefix}\${COMPACTION_IMAGE_PLACEHOLDER}\`;
    const textBudget = Math.max(0, maxCharsPerBlock - placeholder.length);
    return \`\${truncateSummaryText(content, textBudget)}\${placeholder}\`;
}
function serializeMessageForSummary(message, maxCharsPerBlock) {
    ...
    if (message.role === "toolResult") {
        const content = serializeTextBlocks(message.content, Math.min(maxCharsPerBlock, SUMMARIZATION_TOOL_RESULT_CHAR_BUDGET));
