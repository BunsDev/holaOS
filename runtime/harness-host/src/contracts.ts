...
export interface HarnessHostPiRequest {
  ...
  /**
   * Agent's actual cwd. When set, the harness spawns the agent here and uses
   * this as the workspace-boundary root, while still loading workspace.yaml,
   * skills, and other workspace resources from `workspace_dir`. When unset,
   * the agent falls back to running from `workspace_dir`.
   */
  agent_cwd?: string | null;
  session_id: string;
  browser_tools_enabled?: boolean;
  browser_space?: "agent" | null;
  // First-class browser profile the agent drives (mutable at runtime via the
  // browser_use_profile tool). Selects the browser surface; workspace does not.
  browser_profile_id?: string | null;
  force_compaction?: boolean;
  input_id: string;
  instruction: string;
  workflow_owned_subagent?: boolean;
...
function optionalBrowserSpace(
  ...
  // The `user` browser space was retired; treat any legacy value as unset so
  // in-flight or persisted requests still parse instead of throwing.
  if (value === "user") {
    return null;
  }
  throw new Error("browser_space must be `agent`");
}
...
export function decodeHarnessHostPiRequestBase64(encoded: string): HarnessHostPiRequest {
  ...
  return {
    ...
    // Agent's actual cwd. The wire schema carries this as an optional field
    // separate from workspace_dir; without it here every project chat
    // silently falls back to workspace_dir as cwd and bakes the runtime
    // dir into the SDK's pi-session JSONL header — exactly the bug we are
    // fixing across the executor + harness-host plumbing.
    agent_cwd: optionalString(parsed.agent_cwd),
    session_id: requiredString(parsed.session_id, "session_id"),
    ...
    browser_profile_id: optionalString(parsed.browser_profile_id),
    force_compaction: optionalBoolean(parsed.force_compaction, false),
    input_id: requiredString(parsed.input_id, "input_id"),
    instruction: requiredString(parsed.instruction, "instruction"),
    workflow_owned_subagent: optionalBoolean(parsed.workflow_owned_subagent, false),
    ...
    agent_role: optionalString(parsed.agent_role),
  };
}

export const decodePiHarnessHostRequestBase64 = decodeHarnessHostPiRequestBase64;

// Claude Code and Codex carry the same wire payload as pi (see
// runtime/harnesses/src/{claude-code,codex}.ts buildHarnessHostRequest).
// The host runners translate it into CLI invocations, so we reuse pi's
// decoder and expose typed aliases for callers.
export type HarnessHostClaudeCodeRequest = HarnessHostPiRequest;
export type HarnessHostCodexRequest = HarnessHostPiRequest;

export function decodeHarnessHostClaudeCodeRequestBase64(encoded: string): HarnessHostClaudeCodeRequest {
  return decodeHarnessHostPiRequestBase64(encoded);
}

export function decodeHarnessHostCodexRequestBase64(encoded: string): HarnessHostCodexRequest {
  return decodeHarnessHostPiRequestBase64(encoded);
}
