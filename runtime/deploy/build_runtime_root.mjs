...
function stageNodePackage(outputRoot, packageDir, outputName) {
  ...
  copyIfPresent(path.join(packageDir, "tsdown.config.ts"), path.join(targetDir, "tsdown.config.ts"));
  ...
  rmSync(path.join(targetDir, "tsdown.config.ts"), { force: true });
}
...
export function buildRuntimeRoot(outputRootArg = path.join(repoRoot, "out", "runtime-root")) {
  ...
  stageNodePackage(outputRoot, path.join(repoRoot, "packages", "remote-api"), "remote-api");
  // Staged before api-server: its dist must exist for api-server's bundled
  // (noExternal) build to resolve and inline it.
  stageNodePackage(outputRoot, path.join(runtimeRoot, "channel-gateway"), "channel-gateway");
