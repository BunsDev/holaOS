      ...
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <div className="border-t border-border bg-foreground/[0.02] px-4 py-2.5 text-[11px] text-muted-foreground">
            Manage all accounts in Settings → Connectors.
          </div>
        </DialogPrimitive.Popup>
      </DialogPrimitive.Portal>
...
export interface ProviderRowProps {
  ...
  whoami: PendingIntegrationWhoami | null;
  required: boolean;
  /** Fired when the binding state-machine kind changes. Lets a parent (e.g.
   * the install gate) know when a required provider becomes `bound`. */
  onStateChange?: (kind: IntegrationBindingState["kind"]) => void;
}
  ...
  if (state.kind === "bound") {
    ...
    return (
      <div className="mt-0.5 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
        ...
        {lastVerify?.ok ? (
          <span
            className="inline-flex shrink-0 items-center gap-1 text-[10px] text-emerald-600 dark:text-emerald-400"
